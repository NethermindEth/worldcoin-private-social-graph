...
describe('Voting Contract Tests', function () {
    ...
    it('Should not register a candidate twice', async () => {
        ...
        expect(await voting.connect(candidate).registerAsCandidate('Bob')).to.emit(voting, 'UserRegistered');
        const can = await voting.users(candidate.address);
        await expect(voting.connect(candidate).registerAsCandidate('Bob')).to.revertedWith(
            'WorldcoinSocialGraph: INVALID_USER'
        );
    });
    ...
    it('Should recommend a candidate', async () => {
        ...
        const voted = await social_graph.vote(
        ...
        expect(await voting.connect(worldID).recommendCandidate(tx_pour, weight, candidate.address)).to.emit(
            voting,
    ...
    it('Should not recommend a candidate that is not on-chain', async () => {
        ...
        await expect(voting.connect(worldID).recommendCandidate(tx_pour, weight, candidate.address)).to.be.revertedWith(
            'WorldcoinSocialGraph: NOT_A_CANDIDATE'
        );
    });
    ...
    it('Should penalise a malicious voter', async () => {
        ...
        await expect(voting.connect(candidate).penalise()).to.emit(voting, 'Penalised');
    ...
    it('Should update status to verified', async () => {
        ...
        const [candidate, worldID1, worldID2, worldID3, worldID4, worldID5, worldID6, worldID7] =
        ...
        for (var i = 0; i < 7; i++) {
            ...
            expect(await voting.connect(worldIDs[i]).recommendCandidate(tx_pour, weight, candidate.address)).to.emit(
                voting,
                'CandidateRecommended'
            );
        }
        ...
        await expect(voting.connect(candidate).updateStatusVerified(mint_tx)).to.emit(voting, 'CandidateVerified');
    });
    ...
    it('Should allow the user to claim back voting power and get rewards', async () => {
        ...
        for (var i = 0; i < 7; i++) {
            ...
            let voted = await social_graph.vote(
                register.coin,
                old_zcash_address,
                new_zcash_key_pair_1.pk,
                new_zcash_key_pair_2.pk,
        ...
        const claim_rewards = await social_graph.claim(
            old_coin,
            old_address,
            new_zcash_key_pair_1.pk,
            new_zcash_key_pair_2.pk,
        ...
        expect(await voting.connect(worldID1).claimRewards(candidate.address, claim_tx)).to.emit(
            voting,
            'RewardClaimed'
        );
    });
    ...
    it('Should not be able to claim for a non-verified user', async () => {
        ...
        await expect(voting.connect(worldID).claimRewards(candidate.address, tx_pour)).to.be.reverted;
    });
});
